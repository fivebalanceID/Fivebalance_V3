...
- [Development guidelines](#development-guidelines)
    - [General FIVEBALANCE Core](#general-fivebalance-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
FIVEBALANCE Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play FBNs (tFBN)" on the test network, if you
...
FIVEBALANCE Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/fivebalance-project/fivebalance/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_fivebalance
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_fivebalance --log_level=test_suite
$ valgrind -v --leak-check=full src/fivebalanced -printtoconsole
```
...
# A coverage report will now be accessible at `./test_fivebalance.coverage/index.html`.
```
...
- BitcoinMiner : Generates FBNs (if wallet is enabled).
...
However, in open source software such as FIVEBALANCE Core, where everyone uses
...
pay attention to for reviewers of FIVEBALANCE Core code.

General FIVEBALANCE Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: FIVEBALANCE Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against FIVEBALANCE Core so that they can be integrated
...
file descriptors it will cause problems with FIVEBALANCE's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `fivebalance-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `fivebalance-cli` and the GUI debug console use this table to determine how to
