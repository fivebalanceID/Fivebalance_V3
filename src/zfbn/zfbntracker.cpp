...
#include "zfbn/zfbnwallet.h"


CzFBNTracker::CzFBNTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzFBNTracker::~CzFBNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFBNTracker::Init()
{
...
bool CzFBNTracker::Archive(CMintMeta& meta)
{
...
bool CzFBNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFBNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFBNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFBNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzFBNTracker::GetSerialHashes()
{
...
CAmount CzFBNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFBNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFBNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFBNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFBNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFBNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFBNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFBNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFBNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFBNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzFBNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzFBNWallet* zFBNWallet)
{
    bool iszFBNWalletInitialized = (nullptr != zFBNWallet);
    CMintMeta meta;
    ...
    if (!iszFBNWalletInitialized)
        zFBNWallet = new CzFBNWallet(wallet);
    meta.isSeedCorrect = zFBNWallet->CheckSeed(dMint);
    if (!iszFBNWalletInitialized)
        delete zFBNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzFBNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFBNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFBNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFBNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFBNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFBNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
