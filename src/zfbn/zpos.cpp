...
bool CLegacyZFbnStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZFbnStake from non zc-spend", __func__);
    ...
    *this = CLegacyZFbnStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zfbn stake origin", __func__);

    // All good
    return true;
}

CLegacyZFbnStake::CLegacyZFbnStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZFbnStake::GetIndexFrom()
{
...
CAmount CLegacyZFbnStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZFbnStake::GetUniqueness() const
{
...
bool CLegacyZFbnStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zFBN stake block: height %d outside range", __func__, nHeight);
