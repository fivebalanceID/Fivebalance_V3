...
CzFBNWallet::CzFBNWallet(CWallet* parent)
{
    this->wallet = parent;
        ...
        if (walletdb.ReadZFBNSeed_deprecated(seed)) {
            ...
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFBNSeed_deprecated()) {
                    ...
                    LogPrintf("%s: failed to remove old zfbn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfbn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFBNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFBNCount(nCountLastUsed);
    else if (!walletdb.ReadZFBNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFBNWallet::Lock()
{
    seedMaster.SetNull();
}

void CzFBNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFBNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFBN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFBNWallet::LoadMintPoolFromDB()
{
...
void CzFBNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFBNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFBNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zfbnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFBNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFBN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzFBNWallet::SeedToZFBN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzFBNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFBNWallet::GenerateDeterministicZFBN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFBNWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFBNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFBNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
