...
// New serialization introduced with 3.0.99
static const int DBI_OLD_SER_VERSION = 3019900;
static const int DBI_SER_VERSION_NO_ZC = 3019902;   // removes mapZerocoinSupply, nMoneySupply
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action)
    {
        ...
        if (nSerVersion >= DBI_SER_VERSION_NO_ZC) {
            // Serialization with CLIENT_VERSION = 3009902+
            READWRITE(nFlags);
            ...
            // Serialization with CLIENT_VERSION = 3019901
            ...
            // Serialization with CLIENT_VERSION = 3019900-
            int64_t nMint = 0;
            uint256 hashNext{};
            int64_t nMoneySupply = 0;
            READWRITE(nMint);
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action)
    {
        ...
        if (nSerVersion > DBI_OLD_SER_VERSION) {
            // Serialization with CLIENT_VERSION = 3009901
            READWRITE(nMoneySupply);
            READWRITE(nFlags);
            READWRITE(this->nVersion);
            READWRITE(vStakeModifier);
            ...
            // Serialization with CLIENT_VERSION = 3009900-
            READWRITE(nMint);
            READWRITE(nMoneySupply);
            READWRITE(nFlags);
            if (!Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V3_4)) {
