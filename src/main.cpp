...
#if defined(NDEBUG)
#error "FIVEBALANCE cannot be compiled without assertions."
#endif
...

	{
		...
		if (hasZcSpendInputs) {
			...
			if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
				return state.Invalid(error("%s : tried to include zFBN mint output in tx %s",
...
CAmount GetBlockValue(int nHeight)
{
	...
	int64_t nSubsidy = 0;
	if (nHeight == 1) {
		nSubsidy = 486829200 * COIN;
	}
	else if (nHeight < YEARLY_BLOCKCOUNT * 1) {
		nSubsidy = 400 * COIN;
	}
	else if (nHeight <= 983500) {
		nSubsidy = 200 * COIN;
	}
	else if (nHeight > 983500 && nHeight <= 983510) {
		nSubsidy = 30000000 * COIN;
	}
	else if (nHeight > 983510 && nHeight <= 1036811) {
		nSubsidy = 200 * COIN;
	}
	else if (nHeight > 1036811 && nHeight <= 1555211) {
		nSubsidy = 50 * COIN;
	}
	else if (nHeight > 1555211 && nHeight <= 95365771) {
		nSubsidy = 20 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
		...
		for (const CTxIn& in : tx.vin) {
			...
			if (in.IsZerocoinSpend() || isPublicSpend) {
				...
				if (isPublicSpend) {
					...
					if (!ZFBNModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)) {
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
	...
	if (!UpdateZFIVEBALANCEupplyDisconnect(block, pindex)) {
		error("%s: Failed to calculate new zFBN supply", __func__);
		return DISCONNECT_FAILED;
	}
...
{
	util::ThreadRename("fivebalance-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
		...
		if (tx.HasZerocoinSpendInputs()) {
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (isPublicSpend) {
					...
					PublicCoinSpend publicSpend(params);
					if (!ZFBNModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
						return false;
					}
	...
	if (!UpdateZFIVEBALANCEupplyConnect(block, pindex, fJustCheck)) {
		return state.DoS(100, error("%s: Failed to calculate new zFBN supply for block=%s height=%d", __func__,
	...
	if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
		...
		RecalculateFIVEBALANCEupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
	}
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (txIn.IsZerocoinSpend() || isPublicSpend) {
					...
					if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zFBN serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
	...
	if (block.nBits != nBitsRequired) {
		// Fivebalance Specific reference to the block with the wrong threshold was used.
		const Consensus::Params& consensus = Params().GetConsensus();
		if ((block.nTime == (uint32_t)consensus.nFivebalanceBadBlockTime) &&
			(block.nBits == (uint32_t)consensus.nFivebalanceBadBlockBits)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> fbnInputs;
		std::vector<CTxIn> zFBNInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			if (stakeIn.IsZerocoinSpend()) {
				zFBNInputs.push_back(stakeIn);
			}
			else {
				fbnInputs.push_back(stakeIn);
			}
		}
		const bool hasFBNInputs = !fbnInputs.empty();
		const bool hasZFBNInputs = !zFBNInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasFBNInputs) {
					...
					for (const CTxIn& fbnIn : fbnInputs)
						if (fbnIn.prevout == in.prevout)
		...
		if (isBlockFromFork) {
			...
			if (hasZFBNInputs) {
				for (const CTxIn& zFbnInput : zFBNInputs) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zFbnInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfbn");
...
int ActiveProtocol()
{
	// SPORK_14 is used for 70913 (v3.1.1)
	// if (sporkManager.IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
	//         return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

	// SPORK_15 was used for 70915 (v3.2.1), commented out now.
...
{
	{
		...
		if (state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 500000 * Params().GetConsensus().TargetSpacing(pindexBestHeader->nHeight) * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
