...
#include "qt/fivebalance/dashboardwidget.h"
#include "qt/fivebalance/forms/ui_dashboardwidget.h"
#include "qt/fivebalance/sendconfirmdialog.h"
#include "qt/fivebalance/txrow.h"
...
DashboardWidget::DashboardWidget(FIVEBALANCEGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareFbn, "square-chart-fbn");
    ...
    setCssProperty(ui->labelAmountFbn, "text-stake-fbn-disable");
    setCssProperty(ui->labelAmountZfbn, "text-stake-zfbn-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &FIVEBALANCEGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &FIVEBALANCEGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &FIVEBALANCEGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isFbn = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZFBN;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            fbn = (pair.first != 0) ? pair.first / 100000000 : 0;
            zfbn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalFbn += pair.first;
            chartData->totalZfbn += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesFbn.append(fbn);
        chartData->valueszFbn.append(zfbn);

        int max = std::max(fbn, zfbn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(166, 41, 46));
    set1->setColor(QColor(255, 135, 135));
    ...
    set0->append(chartData->valuesFbn);
    set1->append(chartData->valueszFbn);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalFbn > 0 || chartData->totalZfbn > 0) {
        setCssProperty(ui->labelAmountFbn, "text-stake-fbn");
        setCssProperty(ui->labelAmountZfbn, "text-stake-zfbn");
    } else {
        setCssProperty(ui->labelAmountFbn, "text-stake-fbn-disable");
        setCssProperty(ui->labelAmountZfbn, "text-stake-zfbn-disable");
    }
    forceUpdateStyle({ui->labelAmountFbn, ui->labelAmountZfbn});
    ui->labelAmountFbn->setText(GUIUtil::formatBalance(chartData->totalFbn, nDisplayUnit));
    ui->labelAmountZfbn->setText(GUIUtil::formatBalance(chartData->totalZfbn, nDisplayUnit, true));
