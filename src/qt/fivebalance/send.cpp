...
#include "qt/fivebalance/forms/ui_send.h"
#include "qt/fivebalance/addnewcontactdialog.h"
#include "qt/fivebalance/qtutils.h"
#include "qt/fivebalance/sendchangeaddressdialog.h"
#include "qt/fivebalance/optionbutton.h"
#include "qt/fivebalance/sendconfirmdialog.h"
#include "qt/fivebalance/myaddressrow.h"
#include "qt/fivebalance/guitransactionsutils.h"
...
SendWidget::SendWidget(FIVEBALANCEGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-fbn");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination fbnAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(fbnAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(fbnAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
