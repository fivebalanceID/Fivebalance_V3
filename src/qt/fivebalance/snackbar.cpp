...
#include "qt/fivebalance/forms/ui_snackbar.h"
#include "qt/fivebalance/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(FIVEBALANCEGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &FIVEBALANCEGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
