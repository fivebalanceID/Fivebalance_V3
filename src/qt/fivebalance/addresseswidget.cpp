...
#include "qt/fivebalance/forms/ui_addresseswidget.h"
#include "qt/fivebalance/addresslabelrow.h"
#include "qt/fivebalance/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(FIVEBALANCEGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination fbnAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(fbnAdd, label.toUtf8().constData(),
