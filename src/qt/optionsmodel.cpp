...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::FBN);
    ...
    if (settings.contains("nAnonymizeFivebalanceAmount") || reset)
        SoftSetArg("-anonymizefivebalanceamount", settings.value("nAnonymizeFivebalanceAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fivebalance.cpp during shotdown to also remove the window positions
