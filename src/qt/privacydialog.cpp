...
#include "zfivebalancecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zFBN ought to be enough for anybody." - Bill Gates, 2017
    ui->zFBNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFBNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzFBN->setEnabled(false);
        ui->pushButtonMintzFBN->setToolTip(tr("zFBN is currently disabled due to maintenance."));

        ui->pushButtonSpendzFBN->setEnabled(false);
        ui->pushButtonSpendzFBN->setToolTip(tr("zFBN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFBNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFBN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFBN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFBN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFBN in ") +
...
void PrivacyDialog::on_pushButtonSpendzFBN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFBN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zFBN
        sendzFBN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFBN
    sendzFBN();
}

void PrivacyDialog::on_pushButtonZFivebalanceControl_clicked()
{
    ZFivebalanceControlDialog* zFivebalanceControl = new ZFivebalanceControlDialog(this);
    zFivebalanceControl->setModel(walletModel);
    zFivebalanceControl->exec();
}

void PrivacyDialog::setZFivebalanceControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzFivebalanceSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFBN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Fivebalance Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFBNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFBN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FBN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFBN</b>";
    ...
    // use mints from zFivebalance selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZFivebalanceControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZFivebalanceControlDialog::GetSelectedMints();
    }
    ...
    // Clear zfivebalance selector in case it was used
    ZFivebalanceControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFivebalance Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Fivebalance, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFivebalance Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFBN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFBN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFBN "));
    ui->labelzFBNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzFBNSyncStatus->setVisible(fShow);
}
