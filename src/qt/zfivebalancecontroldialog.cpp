...
#include "ui_zfivebalancecontroldialog.h"
...
std::list<std::string> ZFivebalanceControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZFivebalanceControlDialog::listMints;

ZFivebalanceControlDialog::ZFivebalanceControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZFivebalanceControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZFivebalanceControlDialog::~ZFivebalanceControlDialog()
{
    delete ui;
}

void ZFivebalanceControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFivebalanceControlDialog::updateList()
{
...
void ZFivebalanceControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFivebalanceControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZFivebalance_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFivebalanceControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZFivebalanceControlDialog::GetSelectedMints()
{
...
void ZFivebalanceControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
