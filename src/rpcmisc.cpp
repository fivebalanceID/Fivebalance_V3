...
// Copyright (c) 2017-2018 The Fivebalance developers
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    Object zfivebalanceObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zfivebalanceObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zfivebalanceObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zFBNsupply", zfivebalanceObj));
...
Value spork(const Array& params, bool fHelp)
{
    if (params.size() == 1 && params[0].get_str() == "show") {
        Object ret;
        for (int nSporkID = SPORK_STFBN; nSporkID <= SPORK_END; nSporkID++) {
...
#ifdef ENABLE_WALLET
        // Case 1: Fivebalance address and we have full public key:
