...
#include "spork.h"
#include "zfbn/deterministicmint.h"
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                ...
                "\nReturn information about the given FIVEBALANCE address.\n"
                ...
                "  \"account\" : \"account\"                 (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start fivebalanced with -deprecatedrpc=accounts. The account associated with the address, \"\" is the default account\n"
...
UniValue getnewstakingaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new FIVEBALANCE cold staking address for receiving delegated cold stakes.\n"
...
UniValue delegatoradd(const JSONRPCRequest& request)
{
    ...
    if (!keyID)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from FIVEBALANCE address");
...
UniValue delegatorremove(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->HasAddressBook(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get FIVEBALANCE address from addressBook");
...
UniValue listdelegators(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) FIVEBALANCE address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccountaddress (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccountaddress is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new FIVEBALANCE address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "setaccount") {
        if (request.fHelp) {
            throw std::runtime_error("setaccount (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "setaccount is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "setlabel \"fivebalanceaddress\" \"label\"\n"
            "\nSets the label associated with the given address.\n"

            "\nArguments:\n"
            "1. \"fivebalanceaddress\"   (string, required) The fivebalance address to be associated with a label.\n"
...
UniValue getaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccount (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccount is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaddressesbyaccount (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaddressesbyaccount is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }
...
UniValue CreateColdStakeDelegation(const UniValue& params, CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    if (!IsValidDestination(stakeAddr) || !isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid FIVEBALANCE staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid FIVEBALANCE spending address");
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"stakingaddress\"      (string, required) The fivebalance staking address to delegate.\n"
            "2. \"amount\"              (numeric, required) The amount in FBN to delegate for staking. eg 100\n"
            "3. \"owneraddress\"        (string, optional) The fivebalance address corresponding to the key that will be able to spend the stake. \n"
...
UniValue rawdelegatestake(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "     {\n"
            "       \"value\" : x.xxx,            (numeric) The value in FBN\n"
            "       \"n\" : n,                    (numeric) index\n"
            ...
            "           \"fivebalanceaddress\"        (string) fivebalance address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue sendtoaddressix(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"fivebalanceaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "      amount,                 (numeric) The amount in FBN\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "getreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("getreceivedbyaccount (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getreceivedbyaccount is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in FBN received for this label.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    if (request.fHelp ||
        ...
        throw std::runtime_error(
            ...
            "                    To use this deprecated argument, start fivebalanced with -deprecatedrpc=accounts."
            "2. minconf          (numeric, optional, default=1) DEPRECATED. This argument will be removed in v5.0.\n"
            "                    To use this deprecated argument, start fivebalanced with -deprecatedrpc=accounts. Only include transactions confirmed at least this many times.\n"
            "3. includeWatchonly (bool, optional, default=false) DEPRECATED. This argument will be removed in v5.0.\n"
            "                    To use this deprecated argument, start fivebalanced with -deprecatedrpc=accounts. Also include balance in watchonly addresses (see 'importaddress')\n"
            "4. includeDelegated (bool, optional, default=true) Only available when specifying an account.\n"
            "                    To use this argument, start fivebalanced with -deprecatedrpc=accounts. Also include balance delegated to cold stakers\n"
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() > 1 && IsDeprecatedRPCEnabled("accounts")) || (request.params.size() != 0 && !IsDeprecatedRPCEnabled("accounts")))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in FBN received for this account in P2CS contracts.\n"
...
UniValue movecmd(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("move (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "move is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of FBN to move between accounts.\n"
            ...
            "\nMove 0.01 FBN from timotei to akiko with a comment\n" +
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("sendfrom (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "sendfrom is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"tofivebalanceaddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nDEPRECATED (use sendtoaddress). Send an amount from an account to a fivebalance address.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "sendmany \"\" {\"address\":amount,...} ( minconf \"comment\" includeDelegated )\n"
            ...
            "fivebalanced with -deprecatedrpc=accounts\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid FIVEBALANCE address: ")+name_);
...
UniValue listreceivedbylabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "listreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("listreceivedbyaccount (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listreceivedbyaccount is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "listtransactions ( \"dummy\" count from includeWatchonly includeDelegated )\n"
            ...
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in FBN. This is negative and only available for the \n"
...
UniValue listaccounts(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("listaccounts (Deprecated, will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listaccounts is deprecated and will be removed in v5.0. To use this command, start fivebalanced with -deprecatedrpc=accounts.");
    }
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"account\":\"accountname\",       (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start fivebalanced with -deprecatedrpc=accounts. The account name associated with the transaction. Will be \"\" for the default account.\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in FBN. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in FBN\n"
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"account\" : \"accountname\",  (string) DEPRECATED.This field will be removed in v5.0. To see this deprecated field, start fivebalanced with -deprecatedrpc=accounts. The account name involved in the transaction, can be \"\" for the default account.\n"
            "      \"address\" : \"fivebalanceaddress\",   (string) The fivebalance address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in FBN\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending FBNs\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending FBNs\n" +
...
UniValue listunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "3. \"addresses\"    (string) A json array of fivebalance addresses to filter\n"
                "    [\n"
                "      \"address\"   (string) fivebalance address\n"
                "      ,...\n"
                "    ]\n"
                ...
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"address\" : \"address\",  (string) the fivebalance address\n"
                "    \"label\" : \"label\",      (string) The associated label, or \"\" for the default label\n"
                "    \"account\" : \"account\",  (string) DEPRECATED.This field will be removed in v5.0. To see this deprecated field, start fivebalanced with -deprecatedrpc=accounts. Backwards compatible alias for label.\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid FIVEBALANCE address: ") + input.get_str());
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending FBNs.\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total FBN balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the FBN balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the FBN balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in FBN\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in FBN\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in FBN\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in FBN\n"
            ...
            "  \"paytxfee\": x.xxxx                       (numeric) the transaction fee configuration, set in FBN/kB\n"
...
UniValue setstakesplitthreshold(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 FBN each)."
            + HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. value                   (numeric, required) Threshold value (in FBN).\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || (fEnable && request.params.size() != 2) || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same FIVEBALANCE address\n"
...
UniValue getzerocoinbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getzerocoinbalance\n"
            "\nReturn the wallet's total zFBN balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total zFBN balance.\n"
...
UniValue listmintedzerocoins(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nList all zFBN mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    \"zFBN ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zfbnTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (auto m : setMints) {
            ...
            objMint.push_back(Pair("zFBN ID", m.hashPubcoin.GetHex()));     // PubCoin
            ...
            if (m.hashStake.IsNull()) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zfbnTracker->UpdateState(m);
                }
            }
...
UniValue listzerocoinamounts(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zfbnTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "listspentzerocoins\n"
            "\nList all the spent zFBN mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nMint the specified zFBN amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Fbn to convert to zFBN\n"
    ...
    if (!Params().IsRegTestNet())
        throw JSONRPCError(RPC_WALLET_ERROR, "zFBN minting is DISABLED");
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zFBN is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2 || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"value\": amount,         (numeric) Value in FBN.\n"
            "      \"address\": \"xxx\"         (string) FBN address or \"zerocoinmint\" for reminted change.\n"
            "    }\n"
            "    ,...\n"
            "  ]\n"
            "}\n"
    ...
    return DoZfbnSpend(nAmount, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nSpend zFBN mints to a FBN address.\n" +
            HelpRequiringPassphrase() + "\n"
...
extern UniValue DoZfbnSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetmintzerocoin(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzFBNTracker* zfbnTracker = pwalletMain->zfbnTracker.get();
    std::set<CMintMeta> setMints = zfbnTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzFBNTracker* zfbnTracker = pwalletMain->zfbnTracker.get();
    std::set<CMintMeta> setMints = zfbnTracker->ListMints(false, false, false);
    ...
    for (CZerocoinSpend spend : listUnconfirmedSpends) {
        for (auto& meta : setMints) {
            if (meta.hashSerial == GetSerialHash(spend.GetSerial())) {
                zfbnTracker->SetPubcoinNotUsed(meta.hashPubcoin);
...
UniValue exportzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.empty() || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"denomination\"         (integer, optional) Export a specific denomination of zFBN\n"

            "\nResult:\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"id\": \"serial hash\",  (string) the mint's zFBN serial hash \n"
            ...
            "    \"v\": version,   (numeric) The version of the zFBN\n"
            "    \"k\": \"privkey\"  (string) The zFBN private key (V2+ zFBN only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    CzFBNTracker* zfbnTracker = pwalletMain->zfbnTracker.get();
    std::set<CMintMeta> setMints = zfbnTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() == 0)
        throw std::runtime_error(
            ...
            "  \"value\": amount    (numeric) The total zFBN value of zerocoin mints that were added\n"
            "}\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zfbnTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "reconsiderzerocoins\n"
            "\nCheck archived zFBN list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue setzfivebalanceeed(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "setzfivebalanceeed \"seed\"\n"
            "\nSet the wallet's deterministic zfbn seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zfbn seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzfivebalanceeed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzfivebalanceeed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzFBNWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzfivebalanceeed(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "getzfivebalanceeed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zFBN seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzfivebalanceeed", "") + HelpExampleRpc("getzfivebalanceeed", ""));
...
UniValue generatemintlist(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zFBN seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zFBN to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zFBN to generate.\n"
...
UniValue dzfivebalancetate(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
                "dzfivebalancetate\n"
                        "\nThe current state of the mintpool of the deterministic zFBN wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dzfbn_count", nCount));
...
void static SearchThread(CzFBNWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZFBN(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzfbn(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "searchdzfbn\n"
            "\nMake an extended search for deterministically generated zFBN that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zFBN to start with.\n"
            "2. \"range\"       (numeric) How many zFBN to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdzfbn", "1, 100, 2") + HelpExampleRpc("searchdzfbn", "1, 100, 2"));
    ...
    boost::thread_group* dzfbnThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzfbnThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzfbnThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zfbnTracker->GetSerialHashes());
...
UniValue spendrawzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 4 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) FIVEBALANCE address to spend to. If not specified, "
    ...
    return DoZfbnSpend(mint.GetDenominationAsAmount(), vMintsSelected, address_str);
}
