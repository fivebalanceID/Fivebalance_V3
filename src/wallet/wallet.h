...
#include "zfbn/zfbnmodule.h"
#include "zfbn/zfbnwallet.h"
#include "zfbn/zfbntracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_FIVEBALANCE = 61000, // inherited version..
...
// Possible states for zFBN send
enum ZerocoinSpendStatus {
    ZFBN_SPEND_OKAY = 0,                            // No error
    ZFBN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFBN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFBN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFBN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFBN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFBN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFBN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFBN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFBN_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZFBN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFBN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFBN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFBN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFBN_SPENT_USED_ZFBN = 14,                      // Coin has already been spend
    ZFBN_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZFBN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void doZFbnRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZFBN);
    ...
    bool CreateZFBNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zFBN wallet
    CzFBNWallet* zwalletMain{nullptr};
    std::unique_ptr<CzFBNTracker> zfbnTracker{nullptr};
    void setZWallet(CzFBNWallet* zwallet);
    CzFBNWallet* getZWallet();
    ...
    // zFBN reset
    boost::signals2::signal<void()> NotifyzFBNReset;
