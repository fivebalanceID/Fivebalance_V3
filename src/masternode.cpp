...
#define MASTERNODE_COLLATERAL_LOW (1000000 * COIN)
#define MASTERNODE_COLLATERAL_LIMIT_LOW (MASTERNODE_COLLATERAL - 0.01 * COIN)
#define MASTERNODE_COLLATERAL_HIGH (1000000 * COIN)
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 10000 FBN tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 FBN tx -> 1 confirmation
