...
bool CFbnStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CFbnStake from zerocoin spend", __func__);
...
bool CFbnStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CFbnStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CFbnStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CFbnStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CFbnStake::GetValue() const
{
...
CDataStream CFbnStake::GetUniqueness() const
{
...
CBlockIndex* CFbnStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CFbnStake::ContextCheck(int nHeight, uint32_t nTime)
{
