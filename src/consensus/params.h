...
namespace Consensus {
	...
	*/
	...
	struct Params {
		...
		int nTimeSlotLength;
		int64_t nIncreaseBlocktimeHeight;
		...
		// validation by-pass
		int64_t nFivebalanceBadBlockTime;
		unsigned int nFivebalanceBadBlockBits;
		...
		{
			return height <= nIncreaseBlocktimeHeight ? nTargetSpacing : nTargetSpacingV2;
		}
			...
			const int utxoFromBlockHeight, const uint32_t utxoFromBlockTime) const
		{
			...
			if (contextHeight >= nIncreaseBlocktimeHeight && contextHeight < nIncreaseBlocktimeHeight + 5000)
				return (contextHeight - utxoFromBlockHeight >= 40);
			else
