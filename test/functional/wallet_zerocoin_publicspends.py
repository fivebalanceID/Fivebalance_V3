...
class ZerocoinSpendTest(FivebalanceTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zfbn_bal, fbn_bal):
            zfbn_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zfbn_bal)
            fbn_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], fbn_bal)
            return zfbn_bal, fbn_bal
        ...
        zfbn_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zfbn_balance, 6666)
        ...
        zfbn_balance, balance = check_balances(denom_2, zfbn_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zfbn_balance, balance = check_balances(denom_3, zfbn_balance, balance)
        ...
        _, _ = check_balances(denom_1, zfbn_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
