...
class ReorgStakeTest(FivebalanceTestFramework):
    ...
    def check_money_supply(self, expected_fbn, expected_zfbn):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected FBN and zFBN supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_fbn))
            for denom in node['zFBNsupply']:
                assert_equal(node['zFBNsupply'][denom], DecimalAmt(expected_zfbn[denom]))
    ...
    def run_test(self):
        ...
        # Check FBN and zFBN supply at the beginning
        # ------------------------------------------
        # zFBN supply: 2 coins for each denomination
        expected_zfbn_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # FBN supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zfbn_supply)
        ...
        # Verify that FBN and zFBN supplies were properly updated after the spends and reorgs
        self.log.info("Check FBN and zFBN supply...")
        ...
        expected_zfbn_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zfbn_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zfbn_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zfbn_supply)
