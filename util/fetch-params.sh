...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/FIVEBALANCEParams"
    else
        PARAMS_DIR="$HOME/.fivebalance-params"
    fi
fi
...
Failed to fetch the FIVEBALANCE zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
FIVEBALANCE - fetch-params.sh

This script will fetch the FIVEBALANCE zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common FIVEBALANCE zkSNARK parameters. Note that it is
